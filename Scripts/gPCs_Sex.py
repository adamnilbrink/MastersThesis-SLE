...
# Output: association
OUTPUT_ASSOC_ALL = os.path.join(BASE_DIR, "Parallel_Association_All_SNVs_QC_LD.xlsx")
OUTPUT_ASSOC_SELECTED = os.path.join(BASE_DIR, "Parallel_Association_Selected_SNVs_QC_LD.xlsx")

# Output: RF
OUTPUT_RF_ALL = os.path.join(BASE_DIR, "Parallel_RF_All_SNVs_QC_LD.xlsx")
OUTPUT_RF_SELECTED = os.path.join(BASE_DIR, "Parallel_RF_Selected_SNVs_QC_LD.xlsx")

# Output: QC
OUTPUT_QC_XLSX = os.path.join(BASE_DIR, "Parallel_QC_Summary.xlsx")
...
# "important_this_run" = SNP ligger bland top X% importance i den runnen
RF_TOP_FRACTION_PER_RUN = 0.10       # top 10% i varje run.  
MIN_RF_IMPORTANCE_FREQUENCY = 0.50   # vald om viktig i minst 50% av runs
...
required_cols = [IID_COL, LN_COL, TRAIN_COL, EXCL_COL, GENDER_COL]
...
# # # # # # # # # # # # # # # # # # # #
# 9) LADDA PLINK OCH KANDIDAT-SNVs
...
cov_df = df_train[[GENDER_COL] + pc_cols].copy()
