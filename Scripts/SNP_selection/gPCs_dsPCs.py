...
# dsPCs utan age inbäddat
CRAZY_PCA_FILE = os.path.join(BASE_DIR, "SLUTGILTIG", "Cray_PCA_2_noage.xlsx")

# Output: association
OUTPUT_ASSOC_ALL = os.path.join(
    BASE_DIR,
    "Parallel_Association_All_SNVs_StandardPC_plus_CrazyPC_noAgeEmbedded_QC_LD.xlsx"
)
OUTPUT_ASSOC_SELECTED = os.path.join(
    BASE_DIR,
    "Parallel_Association_Selected_SNVs_StandardPC_plus_CrazyPC_noAgeEmbedded_QC_LD.xlsx"
)

# Output: RF
OUTPUT_RF_ALL = os.path.join(
    BASE_DIR,
    "Parallel_RF_All_SNVs_StandardPC_plus_CrazyPC_noAgeEmbedded_QC_LD.xlsx"
)
OUTPUT_RF_SELECTED = os.path.join(
    BASE_DIR,
    "Parallel_RF_Selected_SNVs_StandardPC_plus_CrazyPC_noAgeEmbedded_QC_LD.xlsx"
)

# Output: QC
OUTPUT_QC_XLSX = os.path.join(
    BASE_DIR,
    "Parallel_QC_Summary_StandardPC_plus_CrazyPC_noAgeEmbedded.xlsx"
)
...
def load_crazy_pcs(crazy_pca_file, iid_col="IID", pc_cols=None):
    ...
    if iid_col not in df.columns:
        raise ValueError(f"dsPCs-filen saknar kolumnen '{iid_col}'.")

    missing_pc_cols = [c for c in pc_cols if c not in df.columns]
    if len(missing_pc_cols) > 0:
        raise ValueError(f"dsPCs-filen saknar PC-kolumner: {missing_pc_cols}")
...
def ld_prune_train(G, bim_df, r2_threshold=0.90, window_size=200):
...
def fit_single_snv_logistic(y, snv, cov_df):
    """
    Logistic regression:
    LN ~ SNV + standard gPCs 1-5 + dsPCs 1-2
    """
    X = cov_df.copy()
    X["SNV"] = snv
...
print("Scenario: standard gPCs (1-5) + dsPCs (1-2, no age embedded)")
print("========================================================")
...
# # # # # # # # # # # # # # # # # # # #
# 9) LADDA dsPCs (även kallad Crazy PCs i denna kod)
...
print("Antal saknade dsPC-värden efter merge:")
print(df_train[CRAZY_PC_COLS].isna().sum().to_string())

# # # # # # # # # # # # # # # # # # # #
# 10) LADDA PLINK OCH KANDIDAT-SNVs
...
# # # # # # # # # # # # # # # # # # # #
# 12) FILTERING:  QC + MAF + MISSING + LD
...
print("Scenario: standard PCs (1-5) + crazy PCs (1-2, no age embedded)")
print("========================================================")
