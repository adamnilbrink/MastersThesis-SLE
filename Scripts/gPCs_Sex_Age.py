...
# Output: association
OUTPUT_ASSOC_ALL = os.path.join(BASE_DIR, "Parallel_Association_All_SNVs_with_Age_QC_LD.xlsx")
OUTPUT_ASSOC_SELECTED = os.path.join(BASE_DIR, "Parallel_Association_Selected_SNVs_with_Age_QC_LD.xlsx")

# Output: RF
OUTPUT_RF_ALL = os.path.join(BASE_DIR, "Parallel_RF_All_SNVs_with_Age_QC_LD.xlsx")
OUTPUT_RF_SELECTED = os.path.join(BASE_DIR, "Parallel_RF_Selected_SNVs_with_Age_QC_LD.xlsx")

# Output: QC
OUTPUT_QC_XLSX = os.path.join(BASE_DIR, "Parallel_QC_Summary_with_Age.xlsx")
...
RF_TOP_FRACTION_PER_RUN = 0.10       # top 10% i varje run.  #HÄR
...
print("Scenario: PCs (1-5) + Gender + Age")
print("========================================================")
...
required_cols = [IID_COL, LN_COL, TRAIN_COL, EXCL_COL, GENDER_COL, AGE_COL]
...
df_train[GENDER_COL] = safe_numeric(df_train[GENDER_COL])
df_train[AGE_COL] = safe_numeric(df_train[AGE_COL])
...
print(f"Antal saknade värden i {AGE_COL}: {df_train[AGE_COL].isna().sum()}")

# # # # # # # # # # # # # # # # # # # #
# 8) LADDA PCs

df_pcs = load_eigenvec(EIGENVEC_FILE, n_pcs=N_PCS)
pc_cols = [f"PC{i}" for i in range(1, N_PCS + 1)]

df_train = df_train.merge(df_pcs, on="IID", how="left")
print(f"Antal saknade PC-värden efter merge: {df_train[pc_cols].isna().sum().sum()}")

# # # # # # # # # # # # # # # # # # # #
# 9) LADDA PLINK OCH KANDIDAT SNPs
...
# # # # # # # # # # # # # # # # # # # #
# 13) ASSOCIATIONSSTEG PÅ ALLA QC-PASSADE SNVs
...
# # # # # # # # # # # # # # # # # # # #
# 14) RF-STEG PÅ ALLA QC-PASSADE SNVs
